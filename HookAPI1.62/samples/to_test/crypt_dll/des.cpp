...
int DES::encrypt ( unsigned char key[8], unsigned char* data, unsigned char *data_out, int blocks )
{
   if ((!data)||(blocks<1))
	  return 0;
   deskey ( key, ENCRYPT );
   des ( data, data_out, blocks);
   return 1;
}

int DES::decrypt ( unsigned char key[8], unsigned char* data, unsigned char *data_out, int blocks )
{
   if ((!data)||(blocks<1))
	  return 0;
   deskey ( key, DECRYPT );
...
int DES::yencrypt ( unsigned char key[8], unsigned char* data, int size )
{
   ...
   deskey ( key, ENCRYPT );
   return encrypt ( data, data, data, blocks);
};
...
int DES::ydecrypt ( unsigned char key[8], unsigned char* data, int blocks, int* size )
{
   ...
   deskey ( key, DECRYPT );
   if ( !decrypt ( data, data, data, blocks) )
      return 0;
   if ( size != 0 )
   {
      int pos = blocks*8-1;
